# EE04 / 7.3-inch controller firmware 0.2.0

Controller firmware for this exact hardware combination:

- Seeed Studio XIAO ePaper Display Board EE04;
- XIAO ESP32-S3 Plus;
- 7.3-inch Spectra 6 panel, 800 x 480 pixels, P073_SP6;
- the Home Assistant ePaper Photo Frame App in this repository.

The 7.3-inch 50-pin panel requires EE04. It cannot be connected to the EE02
board intended for the 13.3-inch 60-pin panel. The Home Assistant App performs
photo selection, cropping, colour conversion and dithering. The controller
validates and displays the prepared six-colour RAW frame, reports its state,
and sleeps.

## What version 0.2.0 does

1. Connect to 2.4 GHz Wi-Fi.
2. Read and validate the exact `spectra_7_3_ee04` display profile, geometry,
   frame interval, server time and night window from `/api/device/config`.
3. Select 480 x 800 portrait or 800 x 480 landscape rotation from the server.
4. Download `current.raw` on the first boot and `next.raw` after later wakes.
5. Validate HTTP metadata, exactly 192,000 bytes, 384,000 pixels and every
...
written directly into the Seeed_GFX framebuffer; a second frame buffer is not
allocated.

## Arduino preparation

Use the current setup described by Seeed for EE04 and its ePaper examples. In
Arduino IDE install:
...
The sketch sets Seeed_GFX's official P073_SP6 target and the matching server
profile in `driver.h`:

```cpp
#define BOARD_SCREEN_COMBO 509
#define USE_XIAO_EPAPER_DISPLAY_BOARD_EE04
#define PHOTO_FRAME_DISPLAY_MODEL "spectra_7_3_ee04"
```

In the Home Assistant App configuration select `spectra_7_3_ee04`, save, and
restart the App. The firmware refuses the 13.3-inch profile before initialising
or refreshing this panel.
...
6. With all power disconnected, seat and lock the supplied FPC/adapter cable
   exactly as documented for EE04 and the P073_SP6 panel. Then connect the XIAO
   by USB-C.
7. Open `ee04_photo_frame.ino`, compile, upload, and monitor Serial at
   115200 baud.

A successful first cycle prints Wi-Fi details, the selected rotation, a
validated 192,000-byte count, and the display refresh. It also appears under
...
- confirmation of refresh timing and rotation on the physical P073_SP6 panel;
...
- throughput optimisations based on measured EE04 transfer times.

These features do not change the server protocol and can be added later. For
the 13.3-inch 1200 x 1600 panel, use EE02 and the sibling
`firmware/ee02_photo_frame` sketch.
