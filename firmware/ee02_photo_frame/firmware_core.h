...
constexpr std::uint16_t kPortraitWidth = 1200;
constexpr std::uint16_t kPortraitHeight = 1600;
constexpr std::uint8_t kPaletteSize = 6;

constexpr bool validGeometry(std::uint16_t width, std::uint16_t height) {
  return (width == kPortraitWidth && height == kPortraitHeight) ||
         (width == kPortraitHeight && height == kPortraitWidth);
}
